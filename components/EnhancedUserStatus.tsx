...
import { isPredictionChainSupported } from "@/lib/utils/network-utils";
...
export default function EnhancedUserStatus({
  ...
  const currentChainName = getChainName(currentChainId || 0);
  const isOnSupportedNetwork = isPredictionChainSupported(currentChainId || 0);
  const recommendedChainId = 84532; // Base Sepolia for hackathon

  // Address resolution for better UX
  const {
  ...
  const hasFarcasterAuth =
    user?.authType === "farcaster" || user?.fid || user?.username;
  ...
  const farcasterIdentifier = user?.username
    ? `@${user.username}`
    : user?.fid
    ? `FID: ${user.fid}`
    : "Farcaster";
  // Enhanced wallet display with resolution
  const walletDisplayName =
    hasProfile && resolvedDisplayName
      ? resolvedDisplayName
      : address
  ...
  const primaryDisplayName = hasFarcasterAuth
        ...
        <div className="flex-shrink-0">
          {user?.pfp_url ? (
            ...
            >
              {isWalletOnlyUser
                ? "W"
                : primaryDisplayName.charAt(0).toUpperCase()}
            </div>
          )}
        </div>
            ...
            {hasWalletAuth ? (
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-gray-400">
                    {hasProfile ? "Name:" : "Address:"}
                  </span>
                  <span
                    className={`text-xs ${
                      hasProfile ? "text-blue-300" : "text-gray-300 font-mono"
                    }`}
                  >
                    {walletDisplayName}
                    {isResolvingAddress && (
                  ...
                  {hasProfile && (
                    <span
                      ...
                      title="Resolved from ENS/Basename"
                    >
                      ✓
                    </span>
                  )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    {!isOnSupportedNetwork && (
                      <span
                        ...
                        title="Not optimal for predictions"
                      >
                        ⚠️
                      </span>
                    )}
                  ...
                  {!isOnSupportedNetwork && (
                    <NetworkSwitchButton
                      targetChainId={recommendedChainId}
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  Connect a wallet to stake on predictions and access trading
                  features.
                </p>
              </div>
            )}
...
export function getAuthenticationStatus(
  ...
  return {
    ...
    authLevel:
      hasFarcasterAuth && hasWalletAuth
        ? "dual"
        : hasFarcasterAuth
        ? "social"
        : hasWalletAuth
