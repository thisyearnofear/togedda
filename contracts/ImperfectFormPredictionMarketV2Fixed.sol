...
/**
 * @title ImperfectFormPredictionMarketV2Fixed
 * @dev Fixed version of the prediction market contract for Base Sepolia
...
contract ImperfectFormPredictionMarketV2Fixed {
    ...
    // --- Events (FIXED: Use uint8 instead of Category enum) ---
    ...
    event PredictionCreated(
        ...
        string title,
        uint256 targetDate,
        uint8 category,        // ← FIXED: Changed from Category to uint8
        string network,
        bool autoResolvable
    );
    ...
    event PredictionResolved(
        uint256 indexed predictionId,
        uint8 outcome           // ← FIXED: Changed from Outcome to uint8
    );
        ...
        predictionActive(_predictionId)
    {
        ...
        emit PredictionResolved(_predictionId, uint8(_outcome)); // ← FIXED: Cast enum to uint8
    }
    ...
    function createPrediction(
        ...
        emit PredictionCreated(
            ...
            _title,
            _targetDate,
            uint8(_category),        // ← FIXED: Cast enum to uint8
            _network,
            _autoResolvable
        );
        return predictionId;
        ...
        predictionActive(_predictionId)
    {
        ...
        emit PredictionResolved(_predictionId, uint8(autoOutcome)); // ← FIXED: Cast enum to uint8
    }
    ...
    // Note: charityFeePercentage, maintenanceFeePercentage, charityAddress, and maintenanceAddress
    // are already declared as public state variables, so they automatically have getter functions
}
